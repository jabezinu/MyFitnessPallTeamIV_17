...
export default function ExerciseDatabase() {
  ...
  const handleSearch = async () => {
    setLoading(true)
    try {
      const response = await exerciseAPI.searchExercises(searchQuery, selectedCategory, 50, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && exercises.length === 0 && (searchQuery || selectedCategory) && (
        <div className="text-center py-8">
          <p className="text-gray-500">No exercises found matching your criteria. Try a different search term or category.</p>
        </div>
      )}
