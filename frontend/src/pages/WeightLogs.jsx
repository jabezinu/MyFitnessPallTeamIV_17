...
export default function WeightLogs() {
  ...
  const getTodaysEntry = () => {
    ...
    return logs.find(log => {
      const loggedAtDate = new Date(log.logged_at).toISOString().split('T')[0]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-xl font-semibold mb-6">Other Measurements</h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Last Entry */}
          <div className="space-y-4">
            <h4 className="text-lg font-medium text-gray-900">Last Entry</h4>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Weight:</span>
                <span className="font-medium">{lastEntry ? `${lastEntry.weight_kg} kg` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Neck:</span>
                <span className="font-medium">{lastEntry?.neck_cm ? `${lastEntry.neck_cm} cm` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Waist:</span>
                <span className="font-medium">{lastEntry?.waist_cm ? `${lastEntry.waist_cm} cm` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Hips:</span>
                <span className="font-medium">{lastEntry?.hips_cm ? `${lastEntry.hips_cm} cm` : 'None'}</span>
              </div>
              {lastEntry && (
                <div className="text-sm text-gray-500 mt-2">
                  {new Date(lastEntry.logged_date).toLocaleDateString()}
                </div>
              )}
            </div>
          </div>
          ...
          {/* Today's Entry */}
          <div className="space-y-4">
            <h4 className="text-lg font-medium text-gray-900">Today's Entry</h4>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Weight:</span>
                <span className="font-medium">{todaysEntry ? `${todaysEntry.weight_kg} kg` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Neck:</span>
                <span className="font-medium">{todaysEntry?.neck_cm ? `${todaysEntry.neck_cm} cm` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Waist:</span>
                <span className="font-medium">{todaysEntry?.waist_cm ? `${todaysEntry.waist_cm} cm` : 'None'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Hips:</span>
                <span className="font-medium">{todaysEntry?.hips_cm ? `${todaysEntry.hips_cm} cm` : 'None'}</span>
              </div>
            </div>
          </div>
        </div>

        {/* Track Additional Measurements Form */}
        <div className="mt-8 border-t pt-6">
          <h4 className="text-lg font-medium text-gray-900 mb-4">Track Additional Measurements</h4>

          <form onSubmit={editingLog ? updateWeightLog : addWeightLog} className="space-y-4">
      ...
      {/* Measurement History with Edit functionality */}
      <div className="bg-white p-6 rounded-lg shadow">
